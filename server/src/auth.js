// Crypto + identity for the RUNG Worker: Sign in with Apple verification, opaque
// sessions (D1-backed, revocable), and the HMAC run-anchor used by anti-cheat.
...
// ---- HMAC run-anchor (anti-cheat): ties a /run submission to a real /run/start ----
export async function makeRunToken(env, runId, playerId, dayIndex, serverStartMs) {
  const payload = `${runId}.${playerId}.${dayIndex}.${serverStartMs}`;
  const sig = await hmacHex(payload, env.RUN_SECRET || env.SESSION_SECRET);
  return `${serverStartMs}.${sig}`;
}

export async function verifyRunToken(env, token, runId, playerId, dayIndex) {
  const dot = String(token).indexOf(".");
  if (dot < 0) throw new HttpError(400, "bad_run_token");
  const serverStartMs = Number(token.slice(0, dot));
